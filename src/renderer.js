...
////////////////////////////////////////////////////////////////////////////////
var BaseNetworkRenderer = Object.create(biomart.renderer.results.plain)
var concat = Array.prototype.push

// The wrap container is a div.
BaseNetworkRenderer.tagName = 'div'

BaseNetworkRenderer.addProp = function (node, key, value) {
...
BaseNetworkRenderer.init = function () {
    this.nodes = []
    this.edges = []
    this.rowBuffer = []
    this.neighbors = []
...
/**
 * Creates and inserts new nodes within BaseNetworkRenderer#nodes. If the nodes that are
 * supposed to be created are already present within BaseNetworkRenderer#nodes, then returns
 * those nodes and the new ones.
 *
 ...
 */
BaseNetworkRenderer.insertNodes = function (row, header) {
 ...
 */
BaseNetworkRenderer.insertEdges = function (nodes, row, header) {
...
BaseNetworkRenderer.findIndex = function (collection, cb) {
 ...
 */
BaseNetworkRenderer.makeNE = function (rows) {
...
BaseNetworkRenderer.extend = function(obj) {
    obj.super_ = BaseNetworkRenderer
    obj.prototype = Object.create(BaseNetworkRenderer.prototype, {
        constructor: {
            value: obj,
            enumerable: false,
            writable: true,
    ...
    for (var prop in BaseNetworkRenderer) {
        if (BaseNetworkRenderer.hasOwnProperty(prop))
            obj[prop] = BaseNetworkRenderer[prop]
    }
...
BaseNetworkRenderer.parse = function (rows, writee) {
 ...
 */
BaseNetworkRenderer.newTab = function($container, $tabs) {
    var item, tabNum, svg
 ...
 */
BaseNetworkRenderer.newSVG = function (config) {
    ...
    if ("idName" in config)
        svg.attr("idName", config.idName)
    if ("className" in config)
        svg.attr("className", config.className)
...
BaseNetworkRenderer.printHeader = function(header, writee) {
    this.header = header
}

BaseNetworkRenderer.draw = function (writee) {

}

BaseNetworkRenderer.clear = function () {
    // Now that we're submitting only one query i can safely implement this m.
    this.init()
}

BaseNetworkRenderer.destroy = function () {
    this.init()
}
